...
container_json() { # env image
  ...
  local secrets="["; for s in DATABASE_URL R2_ACCESS_KEY_ID R2_SECRET_ACCESS_KEY KAKAO_REST_API_KEY KAKAO_CLIENT_SECRET KAKAO_PAY_SECRET_KEY KAKAO_MAP_JS_KEY VAPID_PRIVATE_KEY VAPID_SUBJECT CRON_SECRET; do
...
phase_core() {
  ...
  for e in $ENVS; do
    ...
    for s in KAKAO_REST_API_KEY KAKAO_CLIENT_SECRET KAKAO_PAY_SECRET_KEY KAKAO_MAP_JS_KEY VAPID_PRIVATE_KEY VAPID_SUBJECT; do
