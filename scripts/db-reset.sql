-- ============================================================================
-- db-reset.sql — DB public 스키마 완전 비우기
--   public 스키마의 테이블·뷰·머티뷰·시퀀스·함수/프로시저·타입을 전부 제거하고
--   확장(extension) 소속 객체와 롤(app)은 보존(의도).
--
-- 카탈로그 기반(객체명 비-하드코딩)이라 DDL이 늘어도 이 파일은 그대로 쓴다.
-- 이 파일은 "비우기"만 한다. 재생성은 db/schema.sql 정본을 적용한다.
--   → 완전 재세팅 = 이 파일 실행 후 db/schema.sql 적용 (로컬: npm run db:reset).
--
-- 사용: psql -f scripts/db-reset.sql (소유자 롤).
-- ⚠️ public 데이터·객체를 전부 삭제한다. 실행 전 반드시 대상 DB를 확인.
