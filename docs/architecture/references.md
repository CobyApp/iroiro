...
| **인증 (DB 세션 + OAuth 자체 구현)** | [Lucia](https://lucia-auth.com) 세션 가이드 · [카카오 로그인 REST API](https://developers.kakao.com/docs/latest/ko/kakaologin/rest-api) | 쿠키엔 무작위 토큰, DB(`account_session`)엔 SHA-256 해시, 만료·폐기는 행 단위. state(+PKCE) 쿠키로 OAuth 왕복. `modules/auth/`에 구현 — 인증 라이브러리 미사용 |
