...
| **S3 호환 스토리지 통합 패턴** | [dubinc/dub](https://github.com/dubinc/dub) | `lib/storage.ts`의 `aws4fetch` AwsClient로 presign·PUT·GET. `lib/r2/`에 구현(이름은 R2 시절 그대로, 현재 AWS S3·로컬 MinIO) |
| **인증 (DB 세션 + OAuth 자체 구현)** | [Lucia](https://lucia-auth.com) 세션 가이드 · [카카오 로그인 REST API](https://developers.kakao.com/docs/latest/ko/kakaologin/rest-api) · [네이버 로그인 API](https://developers.naver.com/docs/login/api/) | 쿠키엔 무작위 토큰, DB(`account_session`)엔 SHA-256 해시, 만료·폐기는 행 단위. state(+PKCE) 쿠키로 OAuth 왕복. `modules/auth/`에 구현 — 인증 라이브러리 미사용 |
...
| **고객용 상품 이미지 보호** | [Sharp 공식 composite API](https://sharp.pixelplumbing.com/api-composite/) + [Next.js 공식 Route Handlers](https://nextjs.org/docs/app/getting-started/route-handlers) | 고객 화면은 버킷 원본 URL 대신 HMAC 서명된 same-origin GET Route Handler(`app/media/*`)만 사용한다. Node 런타임에서 원본을 축소·반복 워터마크·WebP 재인코딩하고 캐시한다. 관리 화면의 원본 작업은 S3 서명 경로로 유지한다. 상세: [../product-image-protection.md](../product-image-protection.md) |
...
| Better-auth, NextAuth 도입 | inbox-zero / formbricks | 자체 DB 세션·OAuth 구현과 중복 (이력: 초기 Supabase Auth도 제거) |
...
| 관리형 BaaS·서버리스 호스팅 종속 (Supabase Data API/RLS, Vercel 전용 기능, Cloudflare Image Resizing) | 초기 스택 | 컨테이너(ECS) + 일반 Postgres + S3로 이식성 확보. 이력: 2026-08~09 제거 |
...
근거 자료는 대화 히스토리 및 각 GitHub 리포 직접 확인(2026-04-29, Mermaid 항목은 2026-08-06 추가 확인, 인증·스토리지 행은 2026-09 스택 전환 반영).
