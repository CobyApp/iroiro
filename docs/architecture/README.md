...
**formbricks의 `modules/<도메인>/` 컨벤션 + inbox-zero의 `(admin)` 라우트 그룹 + 자체 DB 세션 인증(카카오 OAuth).** 그 외 패턴은 차용하지 않는다.
...
- 의사결정 배경(왜)은 [/docs](../) 상위의 결정 기록 문서(예: [order-checkout-payment.md](../order-checkout-payment.md), [lessons/](../lessons/))에, 룰(어떻게)은 이 폴더에.
