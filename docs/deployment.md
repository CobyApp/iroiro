...
|---|---|---|---|---|
| dev | `dev` | https://dev.iroiro.club | ECS Express `iroiro-dev`(0.5 vCPU / 1 GB) · RDS `iroiro-dev` · S3 `iroiro-kr-products-dev` / `iroiro-kr-ugc-dev` / `iroiro-kr-catalog-dev` | `dev` |
| prd | `main` | https://iroiro.club (+ www) | ECS Express `iroiro-prd`(1 vCPU / 2 GB) · RDS `iroiro-prd` · S3 `iroiro-kr-products-prd` / `iroiro-kr-ugc-prd` / `iroiro-kr-catalog-prd` | `production` |
...
- 커머스 DB는 환경별로 **완전히 분리된 RDS 인스턴스**(PostgreSQL 17, db.t4g.micro, 20GB gp3, 단일 AZ). prd는 삭제 보호 + 7일 백업. 토레카 마스터는 이제 각 환경의 커머스 DB 안에 있어 dev·prd가 독립적으로 가진다(더 이상 공유 카탈로그 DB가 아니다). 보안 그룹은 VPC CIDR(ECS 태스크)만 5432를 열어 두고, 운영자 접속은 SSM 배스천(`i-0ac876386f19fa203`) 포트포워딩으로 한다 — 운영자 IP를 SG에 추가하지 않는다.
...
5. **DB 마이그레이션** — 방금 푸시한 이미지로 원오프 Fargate 태스크 `iroiro-migrate-<env>`(`scripts/db-migrate.mjs`)를 `run-task`해 커머스 DB의 미적용 섹션을 적용한다(`MIGRATE_TARGETS`는 `commerce` 하나). 종료 코드가 0이 아니면 여기서 배포가 멈춘다(서비스는 이전 이미지 유지). 로그는 `/ecs/iroiro-<env>`의 `migrate/…` 스트림.
...
./infra/aws/setup.sh catalog     # 환경별 카탈로그 버킷 + 앱 IAM 정책 + SSM placeholder → ecs, domains 재실행
...
토레카 마스터(team·member·team_member·series·series_kind·card)는 이제 **각 환경의 커머스 DB 안에 있다** — 별도 공유 카탈로그 DB(`iroiro_catalog`)·롤(`catalog_app`)·연결 문자열(`CATALOG_DATABASE_URL`)은 폐기됐다(fix/catalog-merge-into-commerce). dev·prd는 각자 자기 커머스 DB 안에서 토레카 데이터를 독립적으로 가진다. 별도 DB 생성 절차는 필요 없고, 토레카 테이블은 커머스 DB 마이그레이션(`db/schema.sql`의 섹션 → 배포 시 `scripts/db-migrate.mjs`)으로 함께 생성된다.

카탈로그 행의 `card.submitted_by_account_id`는 그 환경의 계정이므로 `submitted_env`를 같이 저장하고, 보상 적립 등은 같은 환경에서만 처리한다. 카드 이미지 버킷은 환경별 `iroiro-kr-catalog-<env>`이며 `./infra/aws/setup.sh catalog`가 만든다.
...
- 정본은 [`db/schema.sql`](../db/schema.sql) 하나(커머스 DB, 토레카 마스터 포함). 변경은 파일 **끝에 `-- [YYYYMMDDHHMMSS_name]` 섹션**으로 덧붙이고 문장은 재실행 안전(IF NOT EXISTS / IF EXISTS)하게 쓴다. 로컬은 `npm run db:reset`으로 재적용.
- dev/prd 적용은 **배포가 자동으로** 한다: `deploy.yml`이 서비스 롤아웃 전에 `iroiro-migrate-<env>` 태스크로 [`scripts/db-migrate.mjs`](../scripts/db-migrate.mjs)를 실행한다. 러너는 커머스 DB의 `schema_migration(name, applied_at, checksum)` 표에 없는 섹션을 파일 순서대로, 섹션마다 한 트랜잭션으로 적용한다. 실패 시 롤백·배포 중단. 손으로 `psql`을 돌리지 않는다.
- **베이스라인**(이미 손으로 적용돼 있던 DB를 러너에 편입할 때 1회): 터널로 소유자 URL을 잡고 `DATABASE_URL_OWNER=… node scripts/db-migrate.mjs --baseline --target commerce [--except <아직 미적용 섹션,…>]`. `--dry-run`으로 먼저 확인. 2026-09-23 dev/prd 커머스 DB에 실행했다.
...
- 새 테이블에는 반드시 `GRANT ... TO app`을 함께 넣는다 — 앱은 비특권 롤로 접속하므로 GRANT가 없으면 permission denied.
