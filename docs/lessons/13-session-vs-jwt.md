...
이로이로는 **어떤 Postgres 위에서도 유효한 인증을 자체 구현**한다 — 카카오·네이버 OAuth로 신원을 받고, 로그인 상태는 우리 DB가 기억한다(2026-06-09 설계, 이후 구현 완료. 이력: 그 전엔 Supabase Auth를 썼다). 그러려면 "로그인 상태를 *무엇으로* 기억하느냐" = **세션 모델**을 직접 골라야 한다.
