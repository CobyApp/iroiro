...
iroiro의 소셜 로그인은 **카카오 전용**이다(네이버는 2026-09 제거). **자체 구현 OAuth**(Supabase Auth·NextAuth 미사용)다. 코드는 이미 구현돼 있으므로, 이 문서의 순서대로 **카카오 콘솔 등록 + 환경변수(또는 SSM 파라미터) 채우기**만 하면 동작한다.

> 관련 코드: [`app/api/auth/[provider]/route.ts`](../app/api/auth/[provider]/route.ts)(시작) · [`app/api/auth/[provider]/callback/route.ts`](../app/api/auth/[provider]/callback/route.ts)(콜백) · [`modules/auth/`](../modules/auth/)(세션·계정·OAuth 클라이언트). 라우트 세그먼트는 `[provider]` 동적이지만 실제 제공자는 카카오뿐이다.
> 인프라·시크릿 주입은 [deployment.md](./deployment.md), 변수 목록은 [environment-variables.md](./environment-variables.md).

## 큰 그림 — 로그인 한 번에 일어나는 일

```
/login 버튼 → GET /api/auth/kakao                   (우리 서버: state·PKCE 쿠키 심고 카카오로 302)
           → 카카오 동의 화면
           → GET /api/auth/kakao/callback            (우리 서버: state 검증 → code 교환 → 프로필)
...
- `redirect_uri`는 코드가 `{APP_URL}/api/auth/kakao/callback`으로 만든다. `APP_URL`이 비어 있으면 **요청 origin으로 폴백**한다(`buildCallbackUrl`은 끝 슬래시를 제거한다).
- state(CSRF)·PKCE 값은 `oauth_state`/`oauth_verifier` 쿠키(httpOnly·Lax·**10분**)에 담긴다. 카카오는 PKCE S256을 사용한다.

## 사업자등록증이 필요한가? — 아니요

카카오 로그인은 개인 개발자(본인인증)만으로 정식 오픈까지 가능하다. **이메일**을 받으려면 비즈 앱이 필요하지만 사업자 없이 "개인 개발자 비즈 앱"으로 대체된다(§1-6).

## 콜백 URL 레퍼런스 (환경별)

콘솔에 등록하는 URI는 코드의 콜백 경로와 **한 글자도 다르면 안 된다**(가장 흔한 실패 원인). 세 환경 모두 등록해 둔다.

| 환경 | `APP_URL` | 카카오 Redirect URI |
|---|---|---|
| 로컬 | `http://localhost:3000` | `http://localhost:3000/api/auth/kakao/callback` |
| dev | `https://dev.iroiro.club` | `https://dev.iroiro.club/api/auth/kakao/callback` |
| prd | `https://iroiro.club` | `https://iroiro.club/api/auth/kakao/callback` |
...
4. **Redirect URI 등록** — [카카오 로그인] → Redirect URI에 위 표의 세 개를 정확히 추가.
...
> **카카오페이 결제**를 함께 쓴다면 카카오페이 콘솔에서 결제용 도메인(사이트 도메인) 등록·Secret Key 발급이 별도로 필요하다 → [order-checkout-payment.md](./order-checkout-payment.md).
...
```bash
for e in dev prd; do
  aws ssm put-parameter --profile coby --region ap-northeast-2 --overwrite --type SecureString \
...
```bash
aws ecs update-service --profile coby --region ap-northeast-2 \
...
# .env.local에 카카오 키 채우기 (위 §2)
npm run dev              # http://localhost:3000
```

확인 순서:

1. `http://localhost:3000/login` → 카카오 버튼이 보인다.
2. 카카오 버튼 → 동의 화면 → 승인 → 신규면 `/signup`(닉네임 입력) → 완료 후 로그인 상태. 기존 신원이면 바로 `/`.
3. `npm run db:studio`로 `account`·`account_identity`·`account_session`(신규 가입 도중엔 `pending_account`) 행 확인.

> **PKCE 라이브 확인**: 카카오에는 PKCE(`code_challenge`)를 함께 보낸다. 첫 로그인이 정상이면 그대로 두고, 카카오가 거부하면 `modules/auth/lib/oauth/kakao.ts`의 `usesPkce: true` → `false` 한 줄만 바꾼다.

## 4. dev / prd 확인

1. SSM 값이 `CHANGE_ME`가 아닌지: `aws ssm get-parameter --with-decryption --name /iroiro/<env>/KAKAO_REST_API_KEY --query Parameter.Value --output text`
...
3. `https://<도메인>/login`에서 실제 로그인 왕복.

## 5. 운영 체크리스트

- [ ] 카카오 콘솔에 로컬·dev·prd 콜백 URI 등록(위 표)
- [ ] SSM `/iroiro/{dev,prd}/{KAKAO_REST_API_KEY,KAKAO_CLIENT_SECRET}` 채움 + 재기동
- [ ] 카카오 Client Secret '사용' 상태 ↔ `KAKAO_CLIENT_SECRET` 값 일치
- [ ] 이메일이 필요하면 카카오 비즈 앱 전환 + `KAKAO_SCOPE`에 `account_email` 추가
- [ ] 로그인 버튼이 카카오 공식 버튼 가이드를 따르는지
...
| 코드 | 의미 | 주요 원인 / 조치 |
|---|---|---|
| `provider_unavailable` | 제공자 비활성 | 카카오 env 키 미설정 → `.env.local` / SSM 확인 |
...
- 2026-06-14 최초 작성(Supabase 로컬 스택·Vercel 기준). AWS ECS/RDS 이전에 맞춰 재작성.
- 2026-09 네이버 로그인 제거 — 카카오 단일로 정리.
