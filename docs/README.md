...
기술 결정·아키텍처·운영 절차를 정리한 문서. 현재 스택: Next.js 16(App Router) · React 19 · Prisma 7 + PostgreSQL 17(AWS RDS) · AWS S3 · AWS ECS Express Mode(Fargate) · 자체 OAuth(카카오, DB 세션) · 카카오페이 결제.
...
- [카카오 로그인 설정](./oauth-setup.md) — 카카오 콘솔 등록 절차, 환경별 콜백 URL, `.env.local`/SSM 값 채우기, 검증·문제 해결
...
- [관리자 페이지 아키텍처](./admin-architecture.md) — site admin(`is_admin`) + 공간별 부분 권한(`admin_roles`) 모델, 분리 시점 기준
- [계정 연동·병합 설계](./account-linking.md) — deferred 가입(`pending_account`), 계정 병합(자동 병합 금지·step-up·감사 기록)
- [장바구니·주문·결제 플로우 설계](./order-checkout-payment.md) — 상태 모델, 재고 원자 차감, 배송비 정책, 카카오페이 결제(`lib/payments` 포트/어댑터)
...
- 그 외 docs/ 루트 문서(`order-checkout-payment.md`·`product-image-protection.md`·`account-linking.md` 등) — **왜** 이 결정을 했는가 (의사결정 기록)
