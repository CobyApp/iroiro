  ...
  return (
    <div className="space-y-5">
      <AdminPageHeader
        ...
        description={
          <>
            ...
            <Link href="/admin/catalog/kinds" className="text-primary underline-offset-2 hover:underline">
              종류 관리
            </Link>
            에서 편집합니다.
          </>
      ...
      <div className="scroll-x scroll-x-fade flex gap-1.5 overflow-x-auto pb-1">
        <Link href="/admin/catalog/series?team=all" className={chip(showAll)}>
          모든 그룹
        </Link>
        {teams.map((t) => (
          <Link key={t.id} href={`/admin/catalog/series?team=${t.id}`} className={chip(teamFilter === t.id)}>
            {t.name}
        ...
        {series.map((s) => (
          <TableRow key={s.id}>
            ...
            <TableCell className="text-right tabular-nums">
              {s.cardCount > 0 ? (
                <Link
                  href={`/admin/catalog/cards?series=${s.id}${s.teamId ? `&team=${s.teamId}` : ""}`}
