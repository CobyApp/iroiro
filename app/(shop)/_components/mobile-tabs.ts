...
// 장바구니는 상단 헤더가 담당하고, 하단은 둘러보기/컬렉션/커뮤니티/마이 4탭이다.
export const TAB_DEFS: TabDef[] = [
  { key: "discover", label: "둘러보기", href: "/" },
  { key: "used", label: "중고거래", href: "/used" },
  { key: "collection", label: "컬렉션", href: "/collections" },
  { key: "community", label: "커뮤니티", href: "/posts" },
  { key: "mypage", label: "마이", href: "/mypage" },
];

// 기능 플래그로 숨길 탭을 제외한 목록 — 중고거래 OFF면 탭 자체가 없다.
export function visibleTabs(options: { usedTradeEnabled: boolean }): TabDef[] {
  return TAB_DEFS.filter(
    (tab) => tab.key !== "used" || options.usedTradeEnabled,
  );
}
