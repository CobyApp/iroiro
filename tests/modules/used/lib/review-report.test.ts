...
// @/lib/db 목 — review-report.ts 는 db as defaultDb 를 기본 인자로 쓴다.
const {
  reportCount,
  reportFindMany,
  reportUpdateMany,
...
function fakeTx() {
  return {
    $queryRaw: txQueryRaw,
    usedReview: { updateMany: txReviewUpdateMany },
    usedReviewReport: { create: txReportCreate, updateMany: txReportUpdateMany },
  };
}
...
vi.mock("@/lib/db", () => ({
  db: {
    ...
    usedReview: { findMany: reviewFindMany, updateMany: reviewUpdateMany },
    $transaction: transaction,
  },
}));

import {
  countUnresolvedUsedReviewReports,
  createUsedReviewReport,
  dismissUsedReviewReport,
  hideUsedReview,
  listUsedReviewReportQueue,
  unhideUsedReview,
} from "@/modules/used/lib/review-report";
...
describe("createUsedReviewReport", () => {
  it("대상 후기가 없거나 숨김이면 신고할 수 없다", async () => {
    txQueryRaw.mockResolvedValue([]);
    await expect(
      createUsedReviewReport(REPORTER, { reviewId: 5, reason: "abuse" }),
    ).rejects.toThrow(/신고할 수 없는 후기/);
  });

  it("본인 후기는 신고할 수 없다", async () => {
    txQueryRaw.mockResolvedValue([reviewRow({ reviewer_account_id: REPORTER })]);
    await expect(
      createUsedReviewReport(REPORTER, { reviewId: 5, reason: "abuse" }),
    ).rejects.toThrow(/본인 후기/);
    expect(txReportCreate).not.toHaveBeenCalled();
  });

  it("정상 신고 — 스냅샷을 동결해 생성한다", async () => {
    txQueryRaw.mockResolvedValue([reviewRow()]);
    await createUsedReviewReport(REPORTER, { reviewId: 5, reason: "false", detail: "거짓" });
  ...
  it("중복 신고(P2002 review_id)는 도메인 메시지로 변환한다", async () => {
    ...
    await expect(
      createUsedReviewReport(REPORTER, { reviewId: 5, reason: "spam" }),
    ).rejects.toThrow(/이미 신고한 후기/);
  });
...
describe("listUsedReviewReportQueue", () => {
  it("대상 후기 상태를 큐 표시 상태로 매핑한다", async () => {
    ...
    const page = await listUsedReviewReportQueue(1);
    expect(page.items.map((i) => i.targetStatus)).toEqual(["visible", "hidden", "missing"]);
    expect(page.items[0].reporterMasked).toBe("#0001");
  });
});

describe("hideUsedReview", () => {
  it("숨김 가능 후기면 hidden 스탬프 + 미해결 신고 일괄 처리", async () => {
    txReviewUpdateMany.mockResolvedValue({ count: 1 });
    txReportUpdateMany.mockResolvedValue({ count: 2 });
    await hideUsedReview("admin-1", { reviewId: 7, reason: "욕설" });
    expect(txReviewUpdateMany.mock.calls[0][0].data.hiddenBy).toBe("admin-1");
    expect(txReportUpdateMany.mock.calls[0][0].data.resolution).toBe("actioned");
  });

  it("숨길 수 없는 상태면 거부하고 신고를 건드리지 않는다", async () => {
    txReviewUpdateMany.mockResolvedValue({ count: 0 });
    await expect(
      hideUsedReview("admin-1", { reviewId: 7, reason: "욕설" }),
    ).rejects.toThrow(/숨길 수 없는/);
    expect(txReportUpdateMany).not.toHaveBeenCalled();
  });
});

describe("unhideUsedReview / dismiss / count", () => {
  it("숨김 후기면 노출로 복원", async () => {
    reviewUpdateMany.mockResolvedValue({ count: 1 });
    await unhideUsedReview("admin-1", 7);
    expect(reviewUpdateMany.mock.calls[0][0].data.hiddenAt).toBeNull();
  });

  it("숨김 상태가 아니면 거부", async () => {
    reviewUpdateMany.mockResolvedValue({ count: 0 });
    await expect(unhideUsedReview("admin-1", 7)).rejects.toThrow(/해제할 수 없는/);
  });

  it("미해결 신고를 dismissed 로 처리", async () => {
    reportUpdateMany.mockResolvedValue({ count: 1 });
    await dismissUsedReviewReport("admin-1", { reportId: 3, note: "무혐의" });
    expect(reportUpdateMany.mock.calls[0][0].data.resolution).toBe("dismissed");
  });

  it("이미 처리된 신고면 거부", async () => {
    reportUpdateMany.mockResolvedValue({ count: 0 });
    await expect(dismissUsedReviewReport("admin-1", { reportId: 3 })).rejects.toThrow(/이미 처리된/);
  });

  it("countUnresolvedUsedReviewReports 는 미해결 수를 센다", async () => {
    reportCount.mockResolvedValue(4);
    expect(await countUnresolvedUsedReviewReports()).toBe(4);
  });
});
