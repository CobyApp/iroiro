...
// 걸리지 않아 .min(1)/.url()에서 실패한다(Vercel 빌드의 ZodError 원인). 선택 필드는
...
const envSchema = z
  .object({
    ...
    R2_BUCKET: z.string().min(1).default("oshikore-products-dev"),
    R2_PUBLIC_BASE: z
      .string()
      .url()
      .default("http://localhost:9000/oshikore-products-dev"),
    ...
    // Supabase 서버 전용 (secret/service_role) 키. RLS 우회 가능 → 클라이언트에 절대 노출 X.
